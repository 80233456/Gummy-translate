...
- 0.4 课堂计时仅统计实际翻译时长，暂停和重连期间不再继续计数
- 课堂页提供横屏双栏布局，旋转时保留字幕、当前句、跟随状态与暂停状态
- 横屏当前句采用双语独立滚动区，长英文不会覆盖中文，并提高横屏字幕信息密度
- 实时当前句保留 Gummy 原生低延迟输出，历史字幕由 Qwen-MT Flash 异步优化后原位替换
- Qwen 优化任务按字幕顺序执行；失败自动回退原译，结束课堂时等待尾部字幕完成
- 竖屏历史区与当前句卡片采用固定边界，长句仅在当前句内部滚动，不再挤压历史字幕
- 保存或丢弃课堂后显式返回首页，不依赖系统返回栈，避免偶发直接退出应用
